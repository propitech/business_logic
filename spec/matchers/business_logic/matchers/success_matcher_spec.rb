...
RSpec.describe BusinessLogic::Matchers::SuccessMatcher do
  ...
  shared_examples "it is a Success" do
    ...
    context "with correct value given" do
      let(:matcher) { matcher_instance.with_value(correct_value) }

      it_behaves_like "matching without error"
    end

    context "with incorrect value given" do
      let(:matcher) { matcher_instance.with_value(incorrect_value) }
      let(:expected_failure_message) do
        "expect to succeed with #{incorrect_value.inspect} but succeeded with #{correct_value.inspect} instead."
      end
  ...
  shared_examples "it is a Failure" do
    ...
    let(:expected_failure_message) { "expect #{actual.inspect} to be a Success" }
