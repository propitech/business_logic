...
<!-- Baseline: agents-baseline-v1.9.0 (source: agents-baseline-v1.9.0)                  -->
...
- Create a commit or file content through the GitHub API (contents or
  git-data endpoints) — API commits land unsigned. Commit locally with `git`
  so the gpg-agent signs it. (`agentic-workflow:signed-commits`)
